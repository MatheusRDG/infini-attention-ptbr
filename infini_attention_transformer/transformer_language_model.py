...
class TransformerLanguageModel(nn.Module):
    def __init__(self, dim_input, num_heads, num_blocks, context_length, max_token_value, dropout=0.1, infini=False, segment_len=4):
        ...
        self.token_embedding_lookup_table = nn.Embedding(num_embeddings=self.max_token_value + 1, embedding_dim=self.dim_input).to('cuda')
        ...
        self.position_encoding_lookup_table = torch.zeros(self.context_length, self.dim_input)
        ...
        # self.position_encoding_lookup_table = self.position_encoding_lookup_table.to('cuda')
    ...
    def forward(self, idx, targets=None):
        ...
        x = self.token_embedding_lookup_table(idx) + self.position_encoding_lookup_table[:T, :].to("cuda")
