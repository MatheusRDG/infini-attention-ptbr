...
class InfiniMultiHeadAttention(nn.Module):
    def __init__(self, num_heads, dim_k, dim_v, dim_input, segment_len, dropout=0.1):
        ...
        self.mask=torch.triu(torch.ones(segment_len, segment_len), diagonal=1).to("cuda")
        self.betas = nn.Parameter(torch.randn(1, num_heads, 1, dim_v))
        self.dropout_layer = nn.Dropout(self.dropout)

        # self.memory:[batch_size, num_heads, dim_k, dim_v]
        # self.memory=torch.zeros(1, num_heads, dim_k, dim_v).to("cuda")

        # self.z:[batch_size, num_heads, 1, dim_k]
        # self.z=(torch.randn(1, num_heads, 1, dim_k)*1e-10).to("cuda")
    ...
    def forward(self, x):
        ...
        memory=torch.zeros(1, self.num_heads, self.dim_k, self.dim_v).to("cuda")
        z=(torch.randn(1, self.num_heads, 1, self.dim_k)*1e-10).to("cuda")
        ...
        for i in range(n_seq):
            ...
            att_mem=self._memory_retrival(memory, q, z)

            self._memory_update(k, v, memory, z)
    ...
    def _memory_retrival(self, memory, q, z):
        ...
        #self.z:[batch_size, num_heads, 1, dim_k]
        #att_mem:[batch_size, num_heads, segment_len, dim_v]
        att_mem = ((sigma_q @ memory) / ((sigma_q @ z.transpose(-2, -1)) + 1e-6)).detach()
        return att_mem
    
    def _memory_update(self, k, v, memory, z):
        #k,v:[batch_size, num_heads, segment_len, dim_k/dim_v]
        sigma_k = nn.functional.elu(k) + 1.0
        # self.memory:[batch_size, num_heads, dim_k, dim_v]
        if memory!=None:
            memory = memory + sigma_k.transpose(-2, -1) @ v
        else:
            memory = sigma_k.transpose(-2, -1) @ v
        
        # self.z:[batch_size, num_heads, 1, dim_k]
        if z!=None:
            z = z + sigma_k.sum(dim=-2, keepdim=True)
        else:
            z = sigma_k.sum(dim=-2, keepdim=True)

    # def _restart_memory(self):
    #     # self.memory:[batch_size, num_heads, dim_k, dim_v]
    #     self.memory=torch.zeros(1, self.num_heads, self.dim_k, self.dim_v).to("cuda")

    #     # self.z:[batch_size, num_heads, 1, dim_k]
    #     self.z=(torch.randn(1, self.num_heads, 1, self.dim_k)*1e-10).to("cuda")
